const LANG_DATA = {
    bm: {
        ...
        tabScan: 'Scan',
        scanPanelTitle: '📷 Scan Nameplate Meter',
        scanDesc: 'Ambil gambar atau upload gambar nameplate meter. App akan baca nilai yang selamat dan minta anda confirm sebelum apply.',
        scanCaptureBtn: 'Ambil Gambar', scanUploadBtn: 'Upload Gambar', scanRunBtn: '🔎 BACA NAMEPLATE',
        scanDetectedTitle: 'Nilai Dikesan', scanApplyBtn: '✅ APPLY NILAI SELAMAT', scanClearBtn: 'Kosongkan Scan', scanRawTitle: 'Teks OCR Mentah',
        scanWarningText: '⚠️ CT Primary / VT Primary / multiplier sebenar <strong>tidak boleh disahkan daripada nameplate meter sahaja</strong>. Sila sahkan daripada nameplate CT/VT, label panel, SLD, sistem bil, atau rekod commissioning.',
        scanNoImage: '❌ Sila pilih gambar dahulu', scanOCRLoading: 'OCR sedang membaca...', scanOCRDone: '✅ Nameplate selesai dibaca', scanOCRError: '❌ OCR gagal. Cuba gambar yang lebih jelas.',
        scanApplied: '✅ Nilai selamat telah diisi ke Calculator', scanCleared: 'Scan dikosongkan',
        scanFieldModel: 'Model', scanFieldMeterType: 'Jenis Meter', scanFieldSupply: 'Supply', scanFieldVoltage: 'Voltage Rating', scanFieldCurrent: 'Current Rating', scanFieldCTSecondary: 'Cadangan CT Secondary', scanFieldActiveConst: 'Constant Active', scanFieldReactiveConst: 'Constant Reactive', scanFieldClass: 'Class', scanFieldFrequency: 'Frequency', scanFieldCTPrimary: 'CT Primary', scanFieldVTPrimary: 'VT Primary', scanFieldMultiplier: 'Multiplier Sebenar',
        scanStatusDetected: 'Dikesan', scanStatusSuggested: 'Cadangan', scanStatusManual: 'Manual', scanStatusNotFound: 'Tidak ditemui', scanManualNeeded: 'Perlu isi manual / sahkan di tapak',
        footerText: 'MeterCalc Pro v3.2 | Dibangunkan oleh <strong>Khirul Anuar</strong> | Untuk <strong>JAMAC Metering Sdn. Bhd.</strong>'
    },
    en: {
        ...
        tabScan: 'Scan',
        scanPanelTitle: '📷 Scan Meter Nameplate',
        scanDesc: 'Capture a photo or upload a meter nameplate image. The app will read safe values and ask you to confirm before applying.',
        scanCaptureBtn: 'Capture Photo', scanUploadBtn: 'Upload Image', scanRunBtn: '🔎 READ NAMEPLATE',
        scanDetectedTitle: 'Detected Values', scanApplyBtn: '✅ APPLY SAFE VALUES', scanClearBtn: 'Clear Scan', scanRawTitle: 'Raw OCR Text',
        scanWarningText: '⚠️ CT Primary / VT Primary / actual multiplier <strong>cannot be confirmed from the meter nameplate alone</strong>. Verify from CT/VT nameplate, panel label, SLD, billing system, or commissioning record.',
        scanNoImage: '❌ Please select an image first', scanOCRLoading: 'OCR is reading...', scanOCRDone: '✅ Nameplate reading complete', scanOCRError: '❌ OCR failed. Try a clearer image.',
        scanApplied: '✅ Safe values applied to Calculator', scanCleared: 'Scan cleared',
        scanFieldModel: 'Model', scanFieldMeterType: 'Meter Type', scanFieldSupply: 'Supply', scanFieldVoltage: 'Voltage Rating', scanFieldCurrent: 'Current Rating', scanFieldCTSecondary: 'Suggested CT Secondary', scanFieldActiveConst: 'Active Constant', scanFieldReactiveConst: 'Reactive Constant', scanFieldClass: 'Class', scanFieldFrequency: 'Frequency', scanFieldCTPrimary: 'CT Primary', scanFieldVTPrimary: 'VT Primary', scanFieldMultiplier: 'Actual Multiplier',
        scanStatusDetected: 'Detected', scanStatusSuggested: 'Suggested', scanStatusManual: 'Manual', scanStatusNotFound: 'Not found', scanManualNeeded: 'Enter manually / verify on site',
        footerText: 'MeterCalc Pro v3.2 | Developed by <strong>Khirul Anuar</strong> | For <strong>JAMAC Metering Sdn. Bhd.</strong>'
    }
};
...
const UIManager = {
    ...
    updateAllLabels() {
        ...
        document.querySelectorAll('.main-tab').forEach(tab => {
            ...
            if (!lbl) return;
            const map = { calculatorPanel: lang === 'bm' ? 'Kalkulator' : 'Calculator', energyPanel: lang === 'bm' ? 'Tenaga' : 'Energy', accuracyPanel: 'Accuracy Test', demandPanel: 'MD', scanPanel: t.tabScan, historyPanel: lang === 'bm' ? 'Sejarah' : 'History', referencePanel: lang === 'bm' ? 'Rujukan' : 'Reference' };
        ...
        setTitle('scanPanel', '📷 Scan Nameplate Meter', '📷 Scan Meter Nameplate');
        ...
        // ============ SCAN RENDER ============
        if (window.Scanner && typeof Scanner.renderDetected === 'function' && Scanner.detected) {
            Scanner.renderDetected();
        }
    },
    ...
    switchMainTab(panelId) {
        ...
        ['calculatorPanel','calcResultsPanel','energyPanel','accuracyPanel','demandPanel','scanPanel','historyPanel','referencePanel'].forEach(id => {
